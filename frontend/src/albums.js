...
export function generatePlaylistHTML(album, album_songs){
    ...
    for(var i = 0; i < album_songs.length; i++){
        ...
        div_tags += `<div className=\"playlist-row-content-container\"><div className=\"play-list-row\" data-track-row=${"\"" + album_songs[i].track_number + "\""}><div className=\"small-toggle-btn\"><i className=\"small-play-btn\"><span className=\"screen-reader-text\">Small toggle button</span></i></div><div className=\"track-number\"><pre className=\"track-number-inner\"> ${album_songs[i].track_number}. </pre></div><div className=\"track-title\"><a className=\"playlist-track\" href=\"#\" data-play-track=${"\"" + album_songs[i].track_number + "\""} style=\"pointer-events: none\">${album_songs[i].name}</a></div></div></div>`;
    }
